...
ARQUIVO_T = os.path.join(DADOS_DIR, "S14.txt")
ARQUIVO_SAIDA = os.path.join(RESULTADOS_DIR, "SB15_14.txt")
...
def encontrar_conjunto_de_cobertura():
    ...
    try:
        ...
        print(f"{len(t_subsets_a_cobrir):,} sequências de tamanho 14 carregadas.")
    ...
    while t_subsets_a_cobrir:
        ...
        for sub_combinacao in itertools.combinations(numeros_restantes, 1):
            k_candidato = tuple(sorted(t_semente + sub_combinacao))
            novos_cobertos = set(itertools.combinations(k_candidato, 14)).intersection(t_subsets_a_cobrir)
