...
ARQUIVO_T = os.path.join(DADOS_DIR, "S12.txt")
ARQUIVO_SAIDA = os.path.join(RESULTADOS_DIR, "SB15_12.txt")
...
def encontrar_conjunto_de_cobertura():
    ...
    try:
        ...
        print(f"{len(t_subsets_a_cobrir):,} sequências de tamanho 12 carregadas.")
    ...
    while t_subsets_a_cobrir:
        ...
        for sub_combinacao in itertools.combinations(numeros_restantes, 3):
            k_candidato = tuple(sorted(t_semente + sub_combinacao))
            novos_cobertos = set(itertools.combinations(k_candidato, 12)).intersection(t_subsets_a_cobrir)
